...
class HMM:
    ...
    def outputPath(self, file, label):
        a_file = open(file, "w")
        for row in self.path:
            a_file.write(label[row] + "\n" )
        a_file.close()
    ...
    def runBaumWelch(self):
        ...
        self.transitionMatrix = trMat
        self.distributions = distrib
        self.stationaryDistribution = self.calcStationaryDistribution(self.transitionMatrix)
    
    def outputParameters(self, outFile):
        file = open(outFile, "w")
