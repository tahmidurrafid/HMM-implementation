...
stDist = calcStationaryDistribution(transitionMatrix)

dp = numpy.full((len(data)+1, 2), -math.inf)
parent = numpy.full((len(data), 2), -math.inf)
path = numpy.zeros(len(data), dtype=int)
dp[0] = calcStationaryDistribution(transitionMatrix)

for i in range(0, stateCount):
    dp[0][i] = math.log(dp[0][i]) + math.log(distributions[i].pdf(data[i]))
print(dp[0])

for j in range(1, len(data)):
    for i in range(0, stateCount):
        for k in range(0, stateCount):
            val = dp[j-1][k] + math.log(transitionMatrix[k][i]*distributions[i].pdf(data[j]))
            print("(" + str(-math.log(distributions[i].pdf(data[j]))) + ")", end=' ')
            if(val >= dp[j][i]):
                dp[j][i] = val
                parent[j][i] = k
        print()
lastValue = -math.inf
for i in range(0, stateCount):
    if(dp[len(data)-1][i] > lastValue):
        lastValue = dp[len(data)-1][i]
        path[len(data)-1] = int(i)
